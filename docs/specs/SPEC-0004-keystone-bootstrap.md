...
- role: `member`;
- catalog: identity (`/v3`) and the implemented image subset (`/v2`).

The catalog does not advertise compute or network services until those routes
exist. Authentication failures use a generic 401 message and never
