...
| #79 | image cache and overlay safety boundary | repository safety boundary plus managed-directory fencing, existing-overlay revalidation, local verified-artifact-to-cache bridge, exact restart cleanup for base/overlay/upload temporaries, regular-target deletion, explicit `qemu-img` qcow2 backing/format verification, and digest/path checks complete; ADR-0087/0104/0115/0135/0142/0147, size/digest/qemu-img revalidation, and symlink/outside-target regression coverage added | blocked until authenticated Glance-to-agent transfer, durable image/overlay ownership, agent-backed image realization, and real-host qemu-img evidence exist; no host evidence claimed; closure evidence: pending |
