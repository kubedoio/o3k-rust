...
| #78 | compute-agent observation completeness / fail-closed real-libvirt profile safety guard | repository protocol slice now fences replaced agent streams before publishing operation, observation, acknowledgement, or error events and durably records authenticated command acceptance as running; ADR-0114/0141/0148 and regression coverage preserve stream identity, operation progress, and duplicate-acceptance idempotency; normal CI also runs `cargo test --workspace --all-features` via ADR-0133 | evidence completeness only: this does not claim durable command replay, command-id storage, the full `o3kd` lifecycle adapter, or real mTLS execution; real-libvirt provider-path and host evidence remain separately blocked; closure evidence: pending |
| #79 | image cache and overlay safety boundary | repository safety boundary plus managed-directory fencing, existing-overlay revalidation, local verified-artifact-to-cache bridge, exact restart cleanup for base/overlay/upload temporaries, regular-target deletion, and declared-qcow2 format verification complete; ADR-0087/0104/0115/0135/0142/0147, size/digest/qemu-img revalidation, and symlink/outside-target regression coverage added | blocked until authenticated Glance-to-agent transfer, durable image/overlay ownership, agent-backed image realization, and real-host qemu-img evidence exist; no host evidence claimed; closure evidence: pending |
