...
| #52 | measurements / release-evidence integrity | merged | fake control-plane measurement now preflights the selected port, verifies launched-PID ownership through readiness/token/RSS checkpoints, and emits redacted diagnostics for port/child failures; release gate now requires separately supplied raw benchmark JSON, environment metadata, summary/raw consistency, and SHA-256 binding; producer-side binding and guest metrics pending |
