...
| #78 | compute-agent observation completeness / fail-closed real-libvirt profile safety guard | repository protocol slice now also fences replaced agent streams before publishing operation, observation, acknowledgement, or error events; ADR-0114/0141 and regression coverage preserve the current connection epoch; normal CI also runs `cargo test --workspace --all-features` via ADR-0133 | evidence completeness only: this does not claim durable command replay, the full `o3kd` lifecycle adapter, or real mTLS execution; real-libvirt provider-path and host evidence remain separately blocked; closure evidence: pending |
...
and cache-hit reuse. Each remains a repository safety decision, not host,
publication,
