...
| #86 | complete real CirrOS OpenStack CLI acceptance | repository-owned public-keypair lifecycle and cleanup now pass the keypair to the server and verify absence for every created resource after CLI deletion; the harness now requires redacted ACTIVE/fixed-IP/config-drive/console-marker acceptance evidence and uses direct `OS_*` configuration without generated credential YAML; ADR-0093 and stateful no-op/special-character regression coverage added | blocked until a protected runner uploads `real-libvirt-e2e.json` with `status: passed`, including real CirrOS ACTIVE/config-drive/console/restart and leak evidence; no host acceptance claimed; closure evidence: pending |
