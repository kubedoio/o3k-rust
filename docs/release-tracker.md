...
| #78 | compute-agent observation completeness / fail-closed real-libvirt profile safety guard | repository protocol slice now fences replaced agent streams, durably journals command identity and payload fingerprints, replays terminal results, converts in-flight journal entries to unknown outcomes on restart, persists command retry budgets, durably fences agent evidence by authenticated command sequence, gates create dispatch on two identity-matched artifact commits, persists transfer metadata/progress/outcomes with epoch fencing, binds resolved create inputs to project/flavor/disk intent, and replays recoverable transfers on provider startup; ADR-0114/0141/0148/0124/0152 and regression coverage preserve stream identity, operation progress, duplicate-acceptance idempotency, restart recovery, bounded artifact transfer, authenticated commit barriers, transfer-journal reopen/replay behavior, and fail-closed project-scoped libvirt realization; normal CI also runs `cargo test --workspace --all-features` via ADR-0133 | evidence completeness only: real mTLS execution, protected-host lifecycle evidence, epoch-change recovery proof, and final real guest proof remain separately blocked; closure evidence: pending |
| #79 | image cache and overlay safety boundary | repository safety boundary plus managed-directory fencing, existing-overlay revalidation, local verified-artifact-to-cache bridge, restart-safe base publication cleanup and cache-base resolution, regular-target deletion, explicit `qemu-img` qcow2 backing/format verification, and digest/path checks complete; ADR-0087/0104/0115/0135/0142/0147, size/digest/qemu-img revalidation, and symlink/outside-target regression coverage added | blocked until authenticated Glance-to-agent transfer, durable image/overlay ownership, agent-backed image realization, and real-host qemu-img evidence exist; no host evidence claimed; closure evidence: pending |
| #80 | config-drive attachment | repository failure cleanup, manifest integrity, digest-bound libvirt attachment, deterministic xorriso ISO materialization with fixed dates, atomic ISO ownership manifests, verified ISO byte reads, typed API request propagation, daemon generation/read-back, and agent-side digest-bound attachment complete; ADR-0088/0105/0055/0116/0149 and regression coverage remove unpublished temporaries, reject altered or ambiguous artifacts before XML generation, and preserve explicit validation of absent inputs | blocked until guest cloud-init consumption, reboot evidence, xorriso protected-host capability, and trusted real-host evidence exist; closure evidence: pending |
| #81 | Neutron/TAP/bridge/DHCP lifecycle | repository link-kind, bounded rollback, authoritative deterministic port-MAC binding, explicit managed dnsmasq lease path, duplicate-MAC fencing, and durable bridge/gateway/TAP ownership manifests with restart validation and foreign-resource protection complete; ADR-0089/0106/0117/0140 and regression coverage include reverse-order cleanup and existing-bridge mutation fencing | blocked until agent-backed create, real TAP/bridge/libvirt/DHCP orchestration, guest fixed-IP evidence, cleanup/restart evidence, and trusted real-host artifact exist; no host acceptance claimed; closure evidence: pending |
| #82 | Placement scheduling and allocations | repository publication rollback, post-delete release-retry, pre-journal create-conflict boundaries, pre-reservation duplicate-name fencing, daemon Placement/scheduler/agent-registry wiring, and authenticated agent inventory publication complete; ADR-0090/0107/0108/0118/0134/0146 and regression coverage restore in-memory state after failed ledger publication, retry release after provider deletion, avoid unnecessary allocation mutation for known conflicts, persist the daemon ledger under its data directory, and project explicit VCPU/MEMORY_MB/DISK_GB capacity with fail-closed agent state | blocked until agent-backed create/delete dispatch, real guest allocation evidence, restart recovery, and trusted real-host scheduling artifact exist; no host acceptance claimed; closure evidence: pending |
| #83 | libvirt lifecycle and observed Nova state | repository observed-state projection and agent observation-state propagation enforce live agent epoch, durable command-sequence evidence fencing, and per-resource observation ordering; ADR-0091/0119/0144/0124 and regression coverage reject delayed or foreign state regressions while successful command observations carry explicit resource state | blocked until agent-backed lifecycle dispatch, real Nova/guest lifecycle evidence, restart/failure recovery, and trusted real-host artifact exist; no host acceptance claimed; closure evidence: pending |
...
| #90 | clean Debian installation and full TestLab lifecycle | repository uninstall precondition ordering, fail-closed preflight disk-space evidence, install/uninstall child-path fencing, reset data/log path fencing, and manifest-backed installer-owned-file protection complete; ADR-0097/0111/0120/0131/0139 and portable packaging coverage reject unsafe paths, foreign overwrites, and unowned removals | host-gated: clean Debian install, dependency/bootstrap validation, real CirrOS lifecycle, reset/reinstall/uninstall/purge, foreign-state preservation, and trusted leak-free `clean-debian-install.json` remain outstanding; no host acceptance claimed; closure evidence: pending |
...
measurement artifact directories. ADR-0139 records manifest-backed ownership
of installer files and child-prefix fencing. ADR-0140 records detailed TAP
kind, managed DHCP lease, and duplicate-MAC boundaries. ADR-0141 records
replaced-agent-stream fencing. ADR-0142 records image-cache directory and
temporary-artifact ownership fences. ADR-0143 records private bounded console
storage. ADR-0144 records durable observation watermarks and live epoch
validation. ADR-0145 records direct durable-cache routing for nonzero console
offsets. ADR-0146 records authenticated agent capability publication into
Placement with an explicit disk-capacity bound and fail-closed state mapping.
ADR-0147 records qemu-img format verification before qcow2 cache publication
and cache-hit reuse. ADR-0148 records durable idempotent command acceptance
progress. Each remains a repository safety decision, not host,
publication,
or independent-human evidence.
