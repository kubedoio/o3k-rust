...
| #78 | compute-agent observation completeness / fail-closed real-libvirt profile safety guard | repository protocol slice now fences replaced agent streams, durably journals command identity and payload fingerprints, replays terminal results, converts in-flight journal entries to unknown outcomes on restart, persists command retry budgets, durably fences agent evidence by authenticated command sequence, and gates create dispatch on two identity-matched artifact commits; ADR-0114/0141/0148/0124/0152 and regression coverage preserve stream identity, operation progress, duplicate-acceptance idempotency, restart recovery, bounded artifact transfer, and authenticated commit barriers; normal CI also runs `cargo test --workspace --all-features` via ADR-0133 | evidence completeness only: the full `o3kd` source resolver, durable transfer journal/restart sweep, real mTLS execution, agent-side libvirt realization, and protected-host evidence remain separately blocked; closure evidence: pending |
