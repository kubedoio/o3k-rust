...
| #78 | compute-agent observation completeness / fail-closed real-libvirt profile safety guard | repository protocol slice now fences replaced agent streams, durably journals command identity and payload fingerprints, replays terminal results, converts in-flight journal entries to unknown outcomes on restart, persists command retry budgets, durably fences agent evidence by authenticated command sequence, gates create dispatch on two identity-matched artifact commits, persists transfer metadata/progress/outcomes with epoch fencing, and binds resolved create inputs to project/flavor/disk intent; ADR-0114/0141/0148/0124/0152 and regression coverage preserve stream identity, operation progress, duplicate-acceptance idempotency, restart recovery, bounded artifact transfer, authenticated commit barriers, transfer-journal reopen behavior, and fail-closed project-scoped libvirt realization; normal CI also runs `cargo test --workspace --all-features` via ADR-0133 | evidence completeness only: durable transfer restart sweep, real mTLS execution, protected-host lifecycle evidence, and final real guest proof remain separately blocked; closure evidence: pending |
