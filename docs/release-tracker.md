...
| #78 | compute-agent observation completeness / fail-closed real-libvirt profile safety guard | repository protocol slice now fences replaced agent streams, durably journals command identity and payload fingerprints, replays terminal results, converts in-flight journal entries to unknown outcomes on restart, persists command retry budgets, durably fences agent evidence by authenticated command sequence, gates create dispatch on two identity-matched artifact commits, and persists transfer metadata/progress/outcomes with epoch fencing; ADR-0114/0141/0148/0124/0152 and regression coverage preserve stream identity, operation progress, duplicate-acceptance idempotency, restart recovery, bounded artifact transfer, authenticated commit barriers, and transfer-journal reopen behavior; normal CI also runs `cargo test --workspace --all-features` via ADR-0133 | evidence completeness only: the full `o3kd` source resolver, durable transfer restart sweep, real mTLS execution, agent-side libvirt realization, and protected-host evidence remain separately blocked; closure evidence: pending |
...
| #80 | config-drive attachment | repository failure cleanup, manifest integrity, digest-bound libvirt attachment, deterministic xorriso ISO materialization with fixed dates, atomic ISO ownership manifests, verified ISO byte reads, and explicit API rejection of unsupported `config_drive: true` requests complete; ADR-0088/0105/0055/0116/0149 and regression coverage remove unpublished temporaries, reject altered or ambiguous artifacts before XML generation, and prevent silent API omission | blocked until Nova/agent artifact wiring, guest cloud-init consumption, reboot evidence, xorriso protected-host capability, and trusted real-host evidence exist; closure evidence: pending |
