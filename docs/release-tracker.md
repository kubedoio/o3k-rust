...
| #90 | clean Debian installation and full TestLab lifecycle | repository uninstall precondition ordering and fail-closed preflight disk-space evidence complete; ADR-0097/0111 and portable packaging coverage ensure rejected purges do not mutate systemd state and missing `df` evidence cannot pass preflight | host-gated: clean Debian install, dependency/bootstrap validation, real CirrOS lifecycle, reset/reinstall/uninstall/purge, foreign-state preservation, and trusted leak-free `clean-debian-install.json` remain outstanding; no host acceptance claimed |
| #91 | real libvirt footprint and lifecycle measurements | repository benchmark evidence boundary complete; ADR-0098/0113 and regression coverage require fresh, identically timestamped, explicitly release-eligible raw and summary benchmark evidence | host-gated: real CirrOS/libvirt measurements, raw samples, host/kernel/libvirt/QEMU/Rust metadata, and `real-libvirt-benchmark.json` with `status: measured` remain outstanding; no host measurement claimed |
