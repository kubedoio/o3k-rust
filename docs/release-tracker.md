...
| #81 | Neutron/TAP/bridge/DHCP lifecycle | draft PR #343 adds the promoted durable host-ownership and TAP/DHCP reconciliation primitives on current main; repository link-kind, bounded rollback, authoritative deterministic port-MAC binding, explicit managed dnsmasq lease path, and duplicate-MAC fencing remain covered | blocked until agent-backed create, real TAP/bridge/libvirt/DHCP orchestration, guest fixed-IP evidence, cleanup/restart evidence, and trusted real-host artifact exist; no host acceptance claimed; closure evidence: pending |
