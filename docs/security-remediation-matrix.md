...
| ASR-013 | medium | Destructive host ownership | dnsmasq cleanup validates argv then signals a reusable numeric PID | #590 | Same-user argv spoof and PID-reuse/process-identity tests on host — DONE: PR #599 (head `176b9d7`) proves ownership by spawn-time identity and signals only the pre-validation pidfd; run `local-01301` on `nkudo-vm1` forced exact numeric PID reuse in a disposable namespace, preserved the same-user argv-spoof canary, terminated the real owned dnsmasq, passed a 100-iteration reap stress with zero wrong signals, and left the foreign canary unchanged (evidence `docs/evidence/asr-013-dnsmasq-pidfd-host-176b9d7.json`; #590 closed) | DHCP, leak, install | Race-resistant handle and exact identity required before signal |
...
| ASR-018 | medium | Durable command/recovery / Placement | Allocation commits before durable consumer intent; startup does not reconcile | #82 | Crash failpoint and startup orphan reconciliation; subsequent create and idempotency on host — DONE: PR #597 (merged `359a188`) adds the after-placement-commit crash failpoint, the fail-closed allocation guard, and in-transaction live-consumer re-checking; run `local-18018` on `nkudo-vm1` reached the exact allocation-committed/intent-absent window, startup reconciliation released the orphan exactly once, the same public create reached ACTIVE with exactly one allocation/domain, and the independent leak verifier passed (evidence `docs/evidence/asr-018-placement-crash-recovery-12a540a.json`; #82 closed) | placement, recovery, leak | No orphan allocation after restart |
