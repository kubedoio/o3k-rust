...
| ASR-015 | closed | Source `124079d` on `nkudo-vm1` captured a real hard-reboot command `accepted` under E1, killed the only non-root compute process after execution but before terminal delivery, registered the same `compute-agent` under E2, and converged the original command and operation to `succeeded`. One execution, one unchanged domain UUID, public console/stop/start/delete, zero post-delete allocations or recoverable commands, independent leak cleanup, and a byte-identical foreign canary were verified. Same-source tests prove stale E1 and queued replaced-epoch evidence cannot mutate durable truth. Evidence: `docs/evidence/asr-015-reconnect-host-124079d.json`; fail-before: `docs/evidence/asr-015-reconnect-host-633f8cb.json` | None for remediation; candidate-bound recertification remains separate |
