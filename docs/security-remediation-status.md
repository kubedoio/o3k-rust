...
|---|---|---|---|
| ASR-001 | closed | Attachment routes are profile-gated, authenticated, and project-scoped with 401/404/409 enforcement, plus the ownership-aware unique-violation mapping that turns a cross-project attach of a foreign volume into a concealed 404 (never 500) and a same-project already-attached volume into a Nova-compatible 409. Fresh two-tenant hosted-Cinder isolation proof on the final head (source `3868905`, runs `local-1786549839` and `local-1786550900` on `nkudo-vm1`, Gazpacho Cinder 28.0.0): token B could not list/show/detach A's attachment or attach A's volume (all 404, zero durable/Cinder/compute/libvirt side effects — durable row attached, iSCSI session present, Cinder volume `in-use` with one attachment), the response never disclosed A's attachment or volume, and tenant B ran its own complete attach/detach lifecycle while A's attachment stayed live; Gate B lifecycle passed and the pinned Tempest volume subset passed (2/2). Portable two-project adversarial matrix added in `crates/o3k-api/tests/attachment_isolation.rs`; route absence for all four methods re-proven in the native profile. Evidence: `docs/evidence/asr-001-002-two-tenant-cinder-3868905.json` | None for remediation; candidate-bound recertification remains separate |
| ASR-002 | closed | Attachment repository lookups are server-scoped before any idempotency response; the cross-project idempotency shape (A owns volume X; B attaches X) resolves to a concealed 404 with no durable row, no Cinder call, and no disclosure of A's attachment id or state — indistinguishable from an unknown volume id (identical status and error envelope). Proven portably (two-project matrix) and on the real hosted-Cinder runs in the same isolation matrix (evidence `docs/evidence/asr-001-002-two-tenant-cinder-3868905.json`). | None for remediation; candidate-bound recertification remains separate |
...
| ASR-013 | closed | PR #599 (head `176b9d7`) makes dnsmasq cleanup prove ownership by the spawn-time identity recorded next to the pidfile and signal only the stable pidfd acquired before validation. Real-host run `local-01301` on `nkudo-vm1` forced exact numeric PID reuse inside a disposable PID namespace and confirmed a stale pidfd never retargets the replacement process; a same-user hostile argv-spoof canary survived cleanup while the real O3K-owned dnsmasq was terminated through the pidfd; a 100-iteration reap stress passed with zero wrong-process signals, zero spoof kills, and zero cleanup failures; the foreign canary digest was unchanged and the independent leak verifier passed. Evidence: `docs/evidence/asr-013-dnsmasq-pidfd-host-176b9d7.json` | None for remediation; candidate-bound recertification remains separate |
...
| ASR-016 | closed | PR #595 (merged `5185586`) implements focused store CAS/identity fencing after main `65600a8` exposed a terminal-operation regression: a stale `Running` projection could overwrite `Succeeded`. The exact final-head host run on `nkudo-vm1` completed 50 overlapping hard-reboot/show iterations with zero terminal regressions, identity drift, duplicate mutations, or stranded state; merged-main CI and Mock Cinder passed on `5185586`. Evidence: `docs/evidence/asr-016-concurrent-durable-evidence-73d4784.json` | None for remediation; candidate-bound recertification remains separate |
...
| ASR-018 | closed | PR #597 (head `12a540a`) adds the after-placement-commit crash failpoint, the fail-closed allocation guard (`insert_resource_and_operation` validates the referenced allocation in the same transaction), and in-transaction live-consumer re-checking in `reconcile_consumers`. Real-host run `local-18018` on `nkudo-vm1` killed o3kd at the exact allocation-committed/intent-absent window: the durable store showed the committed allocation with no resource, operation, agent command, or libvirt domain; startup reconciliation released the orphan exactly once, restored usage to zero, and a repeat startup found no orphans; the same public create then reached ACTIVE with exactly one allocation and one domain; the bounded-inventory exhaustion probe returned the no-valid-host conflict with zero dispatch; the crash-after-intent control case retained the live allocation and converged without reschedule or duplicate mutation; delete released exactly once with usage back to baseline; the independent leak verifier passed and the foreign canary digest was unchanged. Evidence: `docs/evidence/asr-018-placement-crash-recovery-12a540a.json` | None for remediation; candidate-bound recertification remains separate |
...
| ASR-022 | closed | PR #599 head `b96d538` clean-install identity proof on fresh disposable VMs: Ubuntu 24.04 (`nkudo-vm1`-nested KVM, kernel 6.8.0-106, libvirt/QEMU 8.2.2) and Debian 12 bookworm (kernel 6.1.0-52, libvirt 9.0.0, QEMU 7.2.22) both refused pre-existing `o3k` with `kvm`/`libvirt`/unrelated groups and wrong home/shell, and refused `o3k-compute` with an unexpected group, with zero install mutation in every refusal case. Clean installs created distinct UIDs (Ubuntu o3k=997/o3k-compute=995; Debian o3k=994/o3k-compute=993), control `o3k` with no libvirt/kvm membership and no `qemu:///system` access, compute service with `SupplementaryGroups=libvirt kvm` and working polkit rule (Debian), TLS key split (server-key → o3k, agent-key → o3k-compute), state DAC split (`/var/lib/o3k` vs `/var/lib/o3k/compute`), correct systemd identity, and Debian-12-baseline binaries (glibc ≤ 2.34) exec on bookworm. Portable executable regressions added in `tests/packaging-account-refusal.sh` (CI-wired) covering all six contaminated-account cases. Evidence: `docs/evidence/asr-022-service-identity-clean-install-b96d538.json` | None for remediation; candidate-bound recertification remains separate |
