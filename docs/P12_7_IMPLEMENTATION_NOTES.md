...
Native v1 exposes one generic lifecycle precondition: `If-Match:
generation-N` on delete. Ownership is authorized before the generation is
evaluated, and the application checks the durable generation before
idempotency reservation or provider/controller invocation. The black-box
stale-generation, owner-isolation, and zero-provider-side-effect assertions
are in `p12_7_convergence::native_and_openstack_http_surfaces_share_compute_and_network_authority`.
...
| Generation/CAS and authorization-before-side-effect | `p12_7_convergence::native_and_openstack_http_surfaces_share_compute_and_network_authority` exercises `If-Match: generation-0` rejection, owner-before-generation isolation, zero provider mutation, and matching-generation deletion; SQLite and real-PostgreSQL executions use the same HTTP body. `o3k-store` lifecycle/CAS tests provide backend implementation coverage. |
...
| Delegated actor, owner scope, action, target, and parent operation | `p12_6_process::database_controller_and_composition_cross_real_mtls_boundaries` exercises valid mTLS composition plus wrong action, owner scope, parent operation, service principal, expired delegation, a real tenant-B child supplied to the production create boundary, and a replaced-session replay; each rejected case occurs before child relationship mutation |
...
6. idempotency/generation safety — PASS (native HTTP `If-Match: generation-N` stale/matching/foreign-owner evidence in `p12_7_convergence`, executed against SQLite and real PostgreSQL, plus native idempotency replay/conflict/isolation and store CAS tests)
...
5. bounded actor/scope-preserving delegation — PASS (`p12_6_process::database_controller_and_composition_cross_real_mtls_boundaries` independently rejects wrong action, wrong owner scope, wrong parent operation, wrong service principal, expired delegation, a real tenant-B child supplied to create, and a valid old-session delegation after same-endpoint replacement; the test asserts no unauthorized relationship and confirms a replacement-session request succeeds)
