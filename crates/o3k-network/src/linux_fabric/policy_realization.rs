...
#[cfg(test)]
pub(crate) fn test_ip_status(args: &[&str]) -> io::Result<std::process::ExitStatus> {
    std::process::Command::new("ip").args(args).status()
}

#[cfg(test)]
pub(crate) fn test_ip_output(args: &[&str]) -> io::Result<(bool, String)> {
...
mod tests {
    ...
    use std::{io, process::Stdio, sync::Mutex, time::Duration};
