...
use o3k_kernel::{
    ...
    LimitValue, MemoryAuditSink, OwnershipScope, RequiredAuditPublisher, ResourceAmount,
...
use execution::{
    overlay_virtual_size, run_qemu_img, verify_image_format, verify_overlay,
...
#[cfg(test)]
use execution::O3K_TEST_QEMU_IMG_FAIL;
...
"#,
    ...
    fn qemu_img_test_failpoint_env_var_fails_closed() -> Result<(), Box<dyn std::error::Error>> {
        let status = execution::run_failpoint_child()?;
        assert!(
            status.success(),
