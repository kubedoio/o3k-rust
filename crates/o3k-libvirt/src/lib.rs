...
pub fn build_domain_xml(spec: &DomainSpec) -> Result<BuiltDomainXml, LibvirtError> {
    ...
    let xml = format!(
        "<domain type=\"kvm\"><name>{}</name><memory unit=\"MiB\">{}</memory><currentMemory unit=\"MiB\">{}</currentMemory><vcpu>{}</vcpu><metadata><o3k:domain xmlns:o3k=\"{}\" server_id=\"{}\" project_id=\"{}\" generation=\"{}\" operation_id=\"{}\" managed_by=\"{}\" /></metadata><os><type machine=\"pc\">hvm</type></os><devices><controller type=\"scsi\" index=\"0\" model=\"virtio-scsi\" /><controller type=\"pci\" index=\"1\" model=\"pci-bridge\"><address type=\"pci\" domain=\"0x0000\" bus=\"0x00\" slot=\"0x01\" function=\"0x0\" /></controller><serial type=\"file\"><source path=\"{}\" /><target type=\"isa-serial\" port=\"0\" /></serial><console type=\"file\"><source path=\"{}\" /><target type=\"serial\" port=\"0\" /></console><disk type=\"file\" device=\"disk\"><driver name=\"qemu\" type=\"qcow2\" /><source file=\"{}\" /><target dev=\"vda\" bus=\"virtio\" /></disk>{}{}</devices></domain>",
        xml_escape(&name),
        spec.memory_mib,
        spec.memory_mib,
        spec.vcpus,
