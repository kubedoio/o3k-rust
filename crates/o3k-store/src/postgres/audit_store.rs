...
fn row(r: &sqlx::postgres::PgRow) -> Result<AuditEventRecord, StoreError> {
    Ok(AuditEventRecord {
        event_id: r.try_get("event_id").map_err(StoreError::Database)?, timestamp: r.try_get("timestamp").map_err(StoreError::Database)?,
        request_id: r.try_get("request_id").map_err(StoreError::Database)?, audit_id: r.try_get("audit_id").map_err(StoreError::Database)?,
        principal_id: r.try_get("principal_id").map_err(StoreError::Database)?, principal_kind: r.try_get("principal_kind").map_err(StoreError::Database)?,
        effective_scope: r.try_get("effective_scope").map_err(StoreError::Database)?, service: r.try_get("service").map_err(StoreError::Database)?, action: r.try_get("action").map_err(StoreError::Database)?,
        resource_type: r.try_get("resource_type").map_err(StoreError::Database)?, resource_id: r.try_get("resource_id").map_err(StoreError::Database)?, owner_scope: r.try_get("owner_scope").map_err(StoreError::Database)?, operation_id: r.try_get("operation_id").map_err(StoreError::Database)?, outcome: r.try_get("outcome").map_err(StoreError::Database)?, reason_category: r.try_get("reason_category").map_err(StoreError::Database)?,
    })
}
...
impl AuditRepository for PostgresStore {
    ...
    async fn get_audit_event(&self, scope: &str, id: &str) -> Result<AuditEventRecord, StoreError> {
        row(&sqlx::query("SELECT * FROM audit_events WHERE effective_scope=$1 AND event_id=$2").bind(scope).bind(id).fetch_optional(&self.pool).await.map_err(StoreError::Database)?.ok_or(StoreError::ResourceNotFound)?)
    }
    async fn list_audit_events_page(&self, scope: &str, after: Option<&str>, limit: usize) -> Result<RepositoryPage<AuditEventRecord>, StoreError> {
        ...
        let items = rs.iter().take(limit).map(row).collect::<Result<Vec<_>,_>>()?;
        RepositoryPage::new(items, more, key, limit)
    }
    async fn prune_audit_events_before(&self, cutoff: &str, limit: usize) -> Result<u64, StoreError> {
        let n = crate::port::durable::bounded_fetch_limit(limit)?;
