...
impl o3k_native_api::compute::ServerReader for ServerReaderAdapter {
    async fn list_servers_page(
        ...
        let servers = self
            .service
            .list_servers_page(auth.effective_scope().id().as_str(), after_id, limit)
            .await
            .map_err(|error| {
                tracing::error!(%error, "native bounded server list failed");
        ...
        for s in servers {
            ...
            let generation = self
                .service
                .server_generation_for_auth(auth, s.id)
                .await
    ...
    async fn list_servers(
        ...
        match self
            .list_servers_page(auth, None, MAX_LEGACY_ITEMS + 1)
            .await
        {
