...
/// The composition root for the o3kd daemon.  Holds all background task
/// handles, controller session identity, coordination store, and shared
/// application state so that the binary entry point is a thin serve loop.
...
/// Build the daemon composition root from configuration.
///
/// Creates all internal services, background tasks, and application state,
/// returning a [`Composition`] whose [`Composition::shutdown`] method
/// orchestrates the full abort/await/drain sequence.
