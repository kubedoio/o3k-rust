...
umask 077
need_packages=false
command -v xorriso >/dev/null 2>&1 || need_packages=true
  ...
  sudo -n flock -x "$APT_LOCK" bash -c '
    ...
    timeout --signal=TERM --kill-after=30s 300s env DEBIAN_FRONTEND=noninteractive \
      apt-get -o DPkg::Lock::Timeout=300 install -y --no-install-recommends \
        xorriso openssh-client python3-venv pkg-config libvirt-dev protobuf-compiler
...
command -v xorriso >/dev/null 2>&1 || fail "xorriso is unavailable after dependency setup"
