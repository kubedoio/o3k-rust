...
artifacts = {
    "e2e.json": {
        **common,
        ...
        "resources": {name: f"{name}-id" for name in ("image_id", "network_id", "subnet_id", "flavor_id", "server_id")},
        "cleanup": {**common["cleanup"], "resources": {name: "verified_absent" for name in ("image", "network", "subnet", "flavor", "server")}},
