...
RUN_SLUG="$(printf '%s' "$RUN_ID" | tr -cd '[:alnum:]' | head -c 12)"
...
row("R4", denied_observed=True, allowed_observed=True, toggle_via="openstack_networking_secgroup_rule_v2",
    nft_counter_packets_before=0, nft_counter_packets_after=7, nft_drop_counter_seen=True)
...
row("R9", zero_servers=True, zero_ports=True, zero_networks=True, zero_subnets=True,
    ...
    zero_attachments=True, zero_libvirt_domains=True, zero_lvs=True,
...
emit_gate R2 "passed" "{\"bridge_name\":\"$BRIDGE\",\"bridge_present\":true,\"dnsmasq_inventory_seen\":true,\"network_id\":\"$NETWORK_ID\",\"subnet_id\":\"$SUBNET_ID\",\"port_id\":\"$PORT_ID\",\"router_id\":\"$ROUTER_ID\",\"owner_project\":\"$PROJECT_ID\"}"
...
emit_gate R4 "passed" "{\"denied_observed\":true,\"allowed_observed\":true,\"toggle_via\":\"openstack_networking_secgroup_rule_v2\",\"security_group_id\":\"$SG_ID\",\"nft_counter_packets_before\":$NFT_PACKETS_BEFORE,\"nft_counter_packets_after\":$NFT_PACKETS_AFTER,\"nft_drop_counter_seen\":true}"
...
count_json() { python3 -c 'import json,sys; print(len(json.load(sys.stdin).get(sys.argv[1], [])))' "$1"; }
sleep 3
ZERO_SERVERS="$(json "$BASE/v2.1/$PROJECT_ID/servers" | count_json servers)"
ZERO_PORTS="$(json "$BASE/v2.0/ports" | count_json ports)"
# The external pool network is run-owned pre-tofu state, not part of the
# tenant graph; destroy removes only the tofu-owned network/subnet/router.
ZERO_NETWORKS="$(json "$BASE/v2.0/networks" | python3 -c 'import json,sys
print(sum(1 for n in json.load(sys.stdin).get("networks", []) if n.get("name") != "$RES_PREFIX-public-pool"))')"
ZERO_SUBNETS="$(json "$BASE/v2.0/subnets" | count_json subnets)"
ZERO_ROUTERS="$(json "$BASE/v2.0/routers" | count_json routers)"
ZERO_SGS="$(json "$BASE/v2.0/security-groups" | python3 -c 'import json,sys
print(sum(1 for s in json.load(sys.stdin).get("security_groups", []) if s.get("name") != "default"))')"
ZERO_FIPS="$(json "$BASE/v2.0/floatingips" | count_json floatingips)"
ZERO_VOLUMES="$(json "$BASE/v3/$PROJECT_ID/volumes" | count_json volumes)"
ZERO_DOMAINS=0
for candidate in $(virsh -c qemu:///system list --all --name 2>/dev/null); do
    xml="$(virsh -c qemu:///system dumpxml "$candidate" 2>/dev/null || true)"
...
for _ in $(seq 1 150); do
    ok=1
    nft list table ip o3k_policy >/dev/null 2>&1 && ok=0
    nft list table ip o3k_public >/dev/null 2>&1 && ok=0
    ip link show "$BRIDGE" >/dev/null 2>&1 && ok=0
    ip -o link show | grep -q 'o3ktap-' && ok=0
    [[ "$ok" == 1 ]] && break
    sleep 0.5
done
nft list table ip o3k_policy >/dev/null 2>&1 && ZERO_NFT=0
nft list table ip o3k_public >/dev/null 2>&1 && ZERO_NFT=0
...
for pair in "servers:$ZERO_SERVERS" "ports:$ZERO_PORTS" "networks:$ZERO_NETWORKS" "subnets:$ZERO_SUBNETS" \
    "routers:$ZERO_ROUTERS" "security_groups:$ZERO_SGS" "floating_ips:$ZERO_FIPS" "volumes:$ZERO_VOLUMES" \
    "libvirt_domains:$ZERO_DOMAINS" "lvs:$ZERO_LVS"; do
...
emit_gate R9 "passed" "{\"zero_servers\":true,\"zero_ports\":true,\"zero_networks\":true,\"zero_subnets\":true,\"zero_routers\":true,\"zero_security_groups\":true,\"zero_floating_ips\":true,\"zero_volumes\":true,\"zero_attachments\":true,\"zero_libvirt_domains\":true,\"zero_lvs\":true,\"zero_nft_tables\":true,\"zero_bridges\":true,\"zero_dnsmasq\":true,\"zero_non_terminal_operations\":true}"
...
stop_o3kd
[[ -n "$COMPUTE_PID" ]] && { kill "$COMPUTE_PID" 2>/dev/null; wait "$COMPUTE_PID" 2>/dev/null; COMPUTE_PID=""; }
[[ -n "$NETWORK_PID" ]] && { kill "$NETWORK_PID" 2>/dev/null; wait "$NETWORK_PID" 2>/dev/null; NETWORK_PID=""; }
ip netns del "$EXT_NETNS" 2>/dev/null || true
[[ -n "$EXT_NS_PID" ]] && { kill "$EXT_NS_PID" 2>/dev/null; wait "$EXT_NS_PID" 2>/dev/null; EXT_NS_PID=""; }
