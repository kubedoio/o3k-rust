...
start_proxy() { python3 "$root_dir/scripts/p13_5e_fault_proxy.py" --serve-backend "http://127.0.0.1:$backend_port" --listen-port "$proxy_port" --evidence "$work/proxy.json" "$@" >"$work/proxy.address" 2>&1 & proxy_pid=$!; for _ in $(seq 1 50); do curl -fsS "http://127.0.0.1:$proxy_port/readyz" >/dev/null 2>&1 && return; sleep .1; done; }
...
start_proxy --rule 'GET /v2.0/networks* read_response_drop response_loss'; ! "$O3K_P13_TOFU" refresh >/dev/null 2>&1; stop_proxy; start_proxy; "$O3K_P13_TOFU" refresh >/dev/null
sed -i 's/p13-5e-network-a/p13-5e-network-b/' main.tf
stop_proxy; start_proxy --rule 'PUT /v2.0/networks* before_forward pre_forward_failure'; ! "$O3K_P13_TOFU" apply -input=false -auto-approve >/dev/null 2>&1; stop_proxy; start_proxy; "$O3K_P13_TOFU" apply -input=false -auto-approve >/dev/null
stop_proxy; start_proxy --rule 'PUT /v2.0/networks* after_commit_before_response response_loss'; ! "$O3K_P13_TOFU" apply -input=false -auto-approve >/dev/null 2>&1; stop_proxy; start_proxy; "$O3K_P13_TOFU" refresh >/dev/null
stop_proxy; start_proxy --rule 'DELETE /v2.0/networks* after_commit_before_response response_loss'; ! "$O3K_P13_TOFU" destroy -input=false -auto-approve >/dev/null 2>&1; stop_proxy; start_proxy; "$O3K_P13_TOFU" destroy -input=false -auto-approve >/dev/null
